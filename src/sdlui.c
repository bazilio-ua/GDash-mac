...
void
gd_status_line_red(const char *text)
{
    gd_blittext_n(gd_screen, -1, gd_screen->h-gd_font_height(), GD_GDASH_RED, text);
}
...
show_comment(const char *text)
{
    ...
    wrapped=gd_wrap_text(text, gd_screen->w/gd_font_width()-2);
    gd_backup_and_dark_screen();
    lines=g_strsplit_set(wrapped, "\n", -1);
    linenum=g_strv_length(lines);
...
gd_replays_menu(void (*play_func) (GdCave *cave, GdReplay *replay), gboolean for_game)
{
    ...
    if (items->len==0) {
        ...
        gd_backup_and_dark_screen();
        if (for_game)
            gd_status_line("CRSR:MOVE  C:COMMENT  S:SAVED  ESC:EXIT");
        else
        ...
        while (!finished && !gd_quit) {
            ...
            for (n=0; n<lines_per_page && page*lines_per_page+n<items->len; n++) {
                ...
                if (!i->replay) {
                    ...
                    /* player name */
                    g_utf8_strncpy(buffer, i->replay->player_name, 20);    /* name: maximum 20 characters */
                    x=gd_blittext_n(gd_screen, x, y, col, buffer);
                    /* put 20-length spaces */
                    for (c=g_utf8_strlen(buffer, -1); c<20; c++)
                        x=gd_blittext_n(gd_screen, x, y, col, " ");
                    /* always put one space after name */
                    x=gd_blittext_n(gd_screen, x, y, col, " ");
                    ...
                    /* or nothing */
                        comm="-";
                    g_utf8_strncpy(buffer, comm, 11);    /* date: maximum 11 characters */
                    x=gd_blittext_n(gd_screen, x, y, col, comm);
                    /* put 20-length spaces */
                    for (c=g_utf8_strlen(buffer, -1); c<11; c++)
                        x=gd_blittext_n(gd_screen, x, y, col, " ");

                    /* level */                
                    x=gd_blittext_printf_n(gd_screen, x, y, col, " %d", i->replay->level+1);
                    /* comment */                
                    x=gd_blittext_printf_n(gd_screen, x, y, col, "%c", i->replay->comment->len!=0?GD_COMMENT_CHAR:' ');
                    /* saved - check box */                
                    ...
                    switch (event.key.keysym.sym) {
                        ...
                        case SDLK_c:
                            /* show comment */
                            {
                                ...
                                if (i->replay && !g_str_equal(i->replay->comment->str, ""))
                                    show_comment(i->replay->comment->str);
                            }
                            break;
                        case SDLK_s:
